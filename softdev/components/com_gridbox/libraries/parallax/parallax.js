...
!function($){
    ...
    parallax.prototype = {
        ...
        onMouseMove : function(clientX, clientY) {
            var wcx = this.parent.outerWidth() / 2,
                wcy = this.parent.outerHeight() / 2;
            this.ix = (clientX - wcx) / wcx;
            this.iy = (clientY - wcy) / wcy;
            cancelAnimationFrame(this.requestId);
            this.requestId = requestAnimationFrame(this.animate);
        },
                ...
                if (this.options.type == 'mousemove') {
                    ...
                    $(window).on("scroll.parallax"+this.id, function(event){
                        ...
                        if (elemTop < bottom && elemBottom >= top) {
                            //var y = (Math.abs(bottom - elemTop) / 2) * $this.options.offset * ($this.options.invert ? -1.5 : 1.5);
