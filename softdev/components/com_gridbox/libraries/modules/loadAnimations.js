...
app.loadAnimations = function(){
    if (app.viewportItems.length > 0) {
        if (!$g('body').hasClass('gridbox')) {
            ...
            file.rel = 'stylesheet';
            file.href = '{uri_root}components/com_gridbox/libraries/animation/css/animate.css';
            document.getElementsByTagName('head')[0].appendChild(file);
        }
        var file = document.createElement('script');
        file.src = '{uri_root}components/com_gridbox/libraries/animation/js/viewportchecker.js';
        document.getElementsByTagName('head')[0].appendChild(file);
