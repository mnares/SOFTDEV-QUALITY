...
app.menuEditor = function(){
    ...
    if (app.edit.type == 'menu') {
        ...
        value = getValue('dropdown', 'effect', 'animation');
        ...
        value = getValue('dropdown', 'duration', 'animation');
        ...
        value = app.editor.document.querySelectorAll('#'+app.editor.app.edit+' ul li a');
            ...
            $g.ajax({
                ...
                complete: function(msg){
                    ...
                    var item = app.editor.document.getElementById(key),
                        tooltips = item.querySelectorAll('.ba-tooltip');
                    ...
                    for (var i = 0; i < tooltips.length; i++) {
                        app.editor.app.initTooltip($g(tooltips[i]).parent());
                    }
                }
            });
...
function setMenuStyle(type)
{
    ...
    $g('#menu-design-options input[type="range"] + input[data-group="'+type+'"]').each(function(){
        var input = $g(this);
        value = getValue(type, this.dataset.option, this.dataset.subgroup);
        input.val(value);
    ...
    for (var key in app.edit.desktop[type].border) {
        ...
        value = getValue(type, key, 'border');
        switch (key) {
            case 'color' :
