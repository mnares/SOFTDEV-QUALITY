...
app.tabsEditor = function(){
    ...
    if (app.edit.type == 'accordion') {
        li = app.editor.document.querySelectorAll('#'+app.editor.app.edit+' > .accordion > .accordion-group > .accordion-heading .accordion-title');
    }
    ...
    if (app.edit.type == 'tabs') {
        ...
        updateInput(value, getValue('hover', 'color'));
        value = $g('#tabs-settings-dialog [data-option="border"][data-group="header"]');
        updateInput(value, getValue('header', 'border'));
        ...
        updateInput(value, getValue('border', 'color'));
    }
    value = getValue('header', 'color');
