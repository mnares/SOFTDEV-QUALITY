...
class gridboxModelBlogs extends JModelList
{
    ...
    protected function getListQuery()
    {
        ...
        $id = $app->input->get('id', '');
        if ($layout != 'modal' && isset($_GET['category'])) {
            $query = $db->getQuery(true)
                ...
                ->where('`page_category` = '.$db->Quote($_GET['category']))
        ...
        $this->checkThemes();
        $query->select('id, title, theme, published, meta_title, meta_description,
            meta_keywords, intro_image, page_alias, page_category, end_publishing,
            page_access, intro_text, created, language, app_id, hits, order_list')
            ->from('#__gridbox_pages')
            ->where('`app_id` = '.$id)
            ->where('`page_category` <> '.$db->Quote('trashed'));
        if (isset($_GET['category'])) {
            $this->setState('filter.search', '');
            $query->where('`page_category` = '.$db->Quote($_GET['category']));
        }
        $search = $this->getState('filter.search');
        if (!empty($search)) {
            $search = $db->quote('%' . $db->escape($search, true) . '%', false);
